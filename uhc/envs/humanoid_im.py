...
class HumanoidEnv(mujoco_env.MujocoEnv):
    ...
    def __init__(self, cfg, init_expert, data_specs, mode="train", no_root=False):
        ...
        self.set_obs_spaces()
        self.weight = mujoco.mj_getTotalmass(self.model)

    def setup_constants(self, cfg, data_specs, mode, no_root):
        self.cc_cfg = cfg
        self.set_cam_first = set()
        print(self.smpl_robot.export_xml_string().decode("utf-8"))
        import mujoco
        self.smpl_model = mujoco.MjModel.from_xml_string(self.smpl_robot_orig.export_xml_string().decode("utf-8").replace("global", "local"))
        ...
        #     )
        self.sim_model = mujoco.MjModel.from_xml_string(self.smpl_robot.export_xml_string().decode("utf-8").replace("global", "local"))
        self.expert = None
