...
class SMPL_M_Renderer(object):
    ...
    def render_qpose(
        ...
        for fr in range(qpose.shape[0]):
            # import pdb
            # pdb.set_trace()
            self.data.qpos[:] = qpose[fr]
            self.data.qpos[2] += offset_z
    ...
    def show_pose(self, size=(960, 480), loop=False):
        ...
        while True:
            if t >= math.floor(self.T) and not self.qpos_traj is None:
                self.data.qpos[:] = self.qpos_traj[fr % self.qpos_traj.shape[0]]
                self.data.qpos[2] += self.offset_z
                mujoco.mj_forward(self.model, self.data)
                fr += 1
                t = 0
